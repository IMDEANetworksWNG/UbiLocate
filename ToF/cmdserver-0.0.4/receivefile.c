...
#define SERVER_PORT 12000
    ...
    *(uint16_t *) (cmd_dup + 0) = ntohs(T_TFILE);
    *(uint16_t *) (cmd_dup + 2) = ntohs(strlen(remote_file_string));
    ...
    fptr = fopen(local_file_string, "wb");
    if(!fptr) {
    ...
    while(running) {
        chunk ck;
        int to_read = sizeof(chunk);
        int offset = 0;

        while(to_read > 0) {
            int retc = recv(soc, (char *)(&ck) + offset, to_read, 0);
            if(retc == -1) {
                fprintf(stderr, "Receive error or unexpected termination\n");
                goto outError;
            }
            if(retc == 0) {
                goto outError;
        ...
        if(ck.type == C_DATA) {
            fwrite(ck.data, 1, ck.len, fptr);
        }
        else if(ck.type == C_MSG) {
