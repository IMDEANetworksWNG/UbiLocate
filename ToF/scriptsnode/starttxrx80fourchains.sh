...
CONFIG=$(./makecsiparams -e 1 -m 00:12:34:56:78:9b -c 0xe29b -C 0xf -N 0x1 -b 0x88)
LEN=34
...
# start transmitting
./transmit4ever80.sh >/dev/null 2>&1 &

# capture pcap
NODEID=$(ifconfig br0 | grep inet | awk '{ print $2 }' | awk -F"." '{ print $4 }')
./tcpdump -i eth6 -nn -c 2000 -w /tmp/trace${NODEID}.pcap
echo "received 2000 frames"
...
while [ true ]; do
  OUT=$(killall -9 transmit4ever80.sh)
  if [ "$OUT" = "" ]; then
    break
  fi
  usleep 200000
